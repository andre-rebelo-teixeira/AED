...
void AlocaTabelaPalavras(char *ficheiro, st_texto *t)
{
  ...
  {
    (*t).palavras[i] = (char *)malloc((*t).n_total_palavras * sizeof(char));
...
void EscreveFicheiro(char *ficheiro, st_texto *t)
{
  ...
  char *nome;
  char *aux = "w";
  nome = (char *)malloc((strlen(ficheiro) + strlen(".palavras") + 1) * sizeof(char));
  if (nome == NULL)
  {
    fprintf(stderr, "ERROr: not enough memory available!\n");
    exit(5);
  ...
  f = AbreFicheiro(nome, aux);
  for (int i = 0; i < (*t).n_dist_palavras; i++)
  {
 ...
 * ApagarMallocs()
 *
 * Arguments: st_texto - pointer to structure where information is kept
 *            
 * Returns: (none)
 * Side-Effects: none
 *
...
void ApagarMallocs(st_texto *t)
{
  for (unsigned int i = 0; i < (*t).n_total_palavras; i++)
    free((*t).palavras[i]);
