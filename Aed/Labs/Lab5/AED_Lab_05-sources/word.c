...
int AllocWordArray(Tabword *t, char *file)
{
   ...
   (*t) = (Tabword)calloc(nSwords, sizeof(Sword *));
...
int FillInWordArray(Tabword t, char *file)
{
   ...
   while ((locword = ReadWord(f)) != NULL)
   {
      if ((n = NewWord(t, locword, numWords)) == -1)
      {
         ...
         t[numWords] = (Sword *)calloc(1, sizeof(Sword));

         if (t[numWords] == NULL)
            ExitMemError(3);
         t[numWords]->word = (char *)
             calloc((strlen(locword) + 1), sizeof(char));
...
Tabword FreeWordArray(Tabword *t, int numWords)
{
   int i = 0;
