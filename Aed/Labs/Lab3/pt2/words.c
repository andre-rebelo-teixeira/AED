...
t_palavra *criaPalavra(char *pal)
{
  ...
  nova = (t_palavra *)calloc(1, sizeof(t_palavra));
  if (nova == NULL)
    erroMemoria("Reserve memory for new word in criaPalavra");

  nova->pal = (char *)calloc(strlen(pal) + 1, sizeof(char));
  if (nova == NULL)
...
void escreveUmaPalavra(t_palavra *p, FILE *fp, int totalPalavras)
{
  fprintf(fp, "%4d : %s %f\n", p->ocorrencias, p->pal, (float)(p->ocorrencias / (float)totalPalavras));
}
...
{
  libertaString(getPalavra(p));
  free(p);

  return;
}

void printDivertido(t_lista *head, FILE *fpOut, int totalPalavras)
{
  if (head == NULL)
    return;
  printDivertido(getProxElementoLista(head), fpOut, totalPalavras);
  escreveUmaPalavra((t_palavra *)getItemLista(head), fpOut, totalPalavras);
}

void libertaString(char *p)
{
  free(p);
  return;
}